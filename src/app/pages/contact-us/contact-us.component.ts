...
export class ContactUsComponent implements OnDestroy {
  ...
  countryCode!: string;
  countriesOptions = this.countryService.countries.map(
  ...
  contactForm = this.fb.nonNullable.group({
    ...
    message: [
      ...
      [
        ...
        Validators.pattern(
          /^(?=.*[a-zA-Z\u0621-\u064A\u066E\u066F].*[a-zA-Z\u0621-\u064A\u066E\u066F].*[a-zA-Z\u0621-\u064A\u066E\u066F])[a-zA-Z0-9\s\u0621-\u064A\u066E\u066F]+$/,
        ),
      ],
    ],
  });
  ...
  getUserInfo() {
    if (localStorage.getItem('profile')) {
      ...
      this.contactForm.patchValue({
        ...
        lastName:
          this.user.name.replace(this.user.name.split(' ')[0], '') || '',
  ...
  onSubmit() {
    ...
    const message: Message = {
      data: {
        ...
        attributes: {
          name:
            this.contactForm.getRawValue().firstName +
            ' ' +
            this.contactForm.getRawValue().lastName,
    ...
    this.contactUsService.sendMessage(message).subscribe({
      next: (data) => {
        ...
        this.logger.showSuccess(
          LogType.success,
          this.translateService.instant('Message Sent Successfully'),
        );
      },
      error: (error) => {
        this.logger.showSuccess(
